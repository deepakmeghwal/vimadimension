...
const Sidebar = ({ user, onLogout, isCollapsed, toggleSidebar, onUserUpdate }) => {
    ...
    const getCurrentApp = () => {
        ...
        if (path.startsWith('/finance') || path.startsWith('/invoices')) return 'finance';
        if (path === '/profile' || path.startsWith('/profile/')) {
            // Preserve the last app context when on profile page
            return lastAppContextRef.current;
        }
        return 'projects'; // Default for /my-tasks, /projects, /my-approvals, etc.
    };

    // Update the last app context whenever the path changes (but not for profile)
    useEffect(() => {
        const path = location.pathname;
        if (path !== '/profile' && !path.startsWith('/profile/')) {
            if (path.startsWith('/admin')) {
                lastAppContextRef.current = 'admin';
            } else if (path.startsWith('/people')) {
                lastAppContextRef.current = 'people';
            } else if (path.startsWith('/finance') || path.startsWith('/invoices')) {
                lastAppContextRef.current = 'finance';
            } else {
                lastAppContextRef.current = 'projects';
            }
        }
    }, [location.pathname]);
    ...
    // Render navigation based on current app
    const renderNavigation = () => {
        ...
        if (currentApp === 'people') {
            return (
                <div className="nav-group">
                    {!isCollapsed && <p className="nav-label">People</p>}
                    <Link
                        to="/people/directory"
